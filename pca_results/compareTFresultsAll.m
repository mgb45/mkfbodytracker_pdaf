...
name = {'Fixed trajectories (M = 15, d = 21)', 'Fixed trajectories (M = 15, d = 21) + ML', 'Rao-Blackwellised (M = 15, N = 100, d = 21)','Rao-Blackwellised (M = 15, N = 100, d = 21) + ML', ...
    'Optimal MKF (M = 15, N = 60, d = 21)', 'Fixed, PCA trajectories (M = 15, d = 8)','PCA + ML (d = 2)','PCA + ML (d = 4)','PCA + ML (d = 6)','PCA + ML (d = 8)','PCA + ML (d = 10)','PCA + ML (d = 12)'...
    ,'PCA + ML (d = 14)','PCA + ML (d = 16)','PCA + ML (d = 18)','PCA + ML (d = 20)'};
figure;
K = 16;
for k = 1:K
    if (k==1)
    ...
    elseif (k==7)
        load ./PCA_2D_ML/camdata.txt
        load ./PCA_2D_ML/kinectdata.txt   
    elseif (k==8)
        load ./PCA_4D_ML/camdata.txt
        load ./PCA_4D_ML/kinectdata.txt        
    elseif (k==9)
        load ./PCA_6D_ML/camdata.txt
        load ./PCA_6D_ML/kinectdata.txt 
    elseif (k==10)
        load ./PCA_ML/camdata.txt
        load ./PCA_ML/kinectdata.txt
    elseif (k==11)
        load ./PCA_10D_ML/camdata.txt
        load ./PCA_10D_ML/kinectdata.txt
    elseif (k==12)
        load ./PCA_12D_ML/camdata.txt
        load ./PCA_12D_ML/kinectdata.txt
    elseif (k==13)
        load ./PCA_14D_ML/camdata.txt
        load ./PCA_14D_ML/kinectdata.txt
    elseif (k==14)
        load ./PCA2_ML/camdata.txt
        load ./PCA2_ML/kinectdata.txt
    elseif (k==15)
        load ./PCA_18D_ML/camdata.txt
        load ./PCA_18D_ML/kinectdata.txt
    elseif (k==16)
        load ./PCA_20D_ML/camdata.txt
        load ./PCA_20D_ML/kinectdata.txt
    end
